...
data.use.star<- dplyr::filter(data.use.all, subj==star)
...
psa.data <- dplyr::filter(psa.data.all, !(subj==star & age>=last.age))
data.use <- dplyr::filter(data.use.all, !(subj==star & age>=last.age))
